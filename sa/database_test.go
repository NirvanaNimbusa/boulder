...
func TestMaxOpenConns(t *testing.T) {
	...
	_, err := NewDbMap("sa@tcp(boulder-mysql:3306)/boulder_sa_integration", 100, nil)
	if err != nil {
...
func TestNewDbMap(t *testing.T) {
	...
	dbMap, err := NewDbMap(mysqlConnectURL, 0, nil)
	if err != errExpected {
...
func TestStrictness(t *testing.T) {
	dbMap, err := NewDbMap(vars.DBConnSA, 1, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTimeouts(t *testing.T) {
	dbMap, err := NewDbMap(vars.DBConnSA+"?readTimeout=1s", 1, nil)
	if err != nil {
...
func TestAutoIncrementSchema(t *testing.T) {
	dbMap, err := NewDbMap(vars.DBInfoSchemaRoot, 1, nil)
